...
class DoctorTests(unittest.TestCase):
    ...
    def test_reports_skill_copy_and_runtime_version_drift(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(report["runtime_status"]["versions"]["actual_runtime"], "0.7.1a1")
    ...
    def test_installed_runtime_without_codex_registration_is_degraded(self):
        with tempfile.TemporaryDirectory() as temp:
            home = Path(temp)
            runtime = home / ".intent-translator" / "mcp" / "runtimes" / "0.7.1a1" / "venv" / "Scripts"
            runtime.mkdir(parents=True)
            command = runtime / "intent-translator-mcp.exe"
            command.touch()
            (home / ".intent-translator" / "mcp" / "current.json").write_text(
                json.dumps({"version": "0.7.1a1", "command": str(command)}),
                encoding="utf-8",
            )
            registration = {
                "host": "codex",
